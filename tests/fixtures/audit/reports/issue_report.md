...
|---|-----------|---------|----------|
| 1 | Tests assert that `audit_runner.py issue <id>` and `audit_runner.py project` subcommands exist and parse the flags `--persist`, `--pi-bin`, and `--model`. | met | tests/test_audit_runner_core.py:43 — TestCLIParsing class covers all CLI flag combinations with explicit assertions |
