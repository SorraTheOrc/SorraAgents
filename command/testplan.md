---
description: Design an automated end-to-end test plan and create test-case work items
tags:
  - workflow
  - testplan
  - qa
...
- Run `wl show <work-item-id> --json` and summarise the work item in one paragraph: title, type (epic/feature/task), headline, acceptance criteria, and any existing plan/milestone links.
- Read any PRD or plan linked in this work item or its parents to extract key user flows, acceptance criteria, and constraints.
- Read the documentation identified in the sibling work item which focuses on documentation for this feature.
- Identify the **test implementation work item** id to use for creating test-case children (see Quick Inputs, above). If ambiguous, stop and ask for clarification, asking the user to confirm they ran the `/plan` command, before proceeding.
...
- Idempotence rules:
  - Before creating, fetch existing children: `wl list --parent <testWorkItemId> --json` and reuse any child whose canonical title matches the planned test case.
  - Do not create duplicates; update descriptions instead when reuse occurs.
- Update the **test implementation work item** description to add or replace a well-marked "Test Plan" block that lists the approved test cases, their work item ids (created/reused), automation levels, and Open Questions.
- If helpful, add dependencies to reflect ordering (e.g., e2e blocked by fixtures): `wl dep add <DependentTestCaseId> <PrereqTestCaseId>`.

## Traceability & idempotence

- Re-running `/testplan <work-item-id>` should not create duplicate test-case work items or duplicate "Test Plan" blocks.
- If the command makes changes, include a changelog block in the test work item (labelled "Test Plan: changelog") summarising actions and timestamps.
...
- **Worklog validation**: ensure created test-case descriptions include a `## Acceptance Criteria` section to satisfy validator expectations.
