...
**Concurrency:** `--max-concurrency N` bounds the number of concurrent pi/audit subprocesses host-wide (default: `AUDIT_MAX_CONCURRENCY` env var or 2). Each pi launch holds one audit slot; if the ceiling is saturated past the bounded wait, the call returns `unmet` with evidence "Audit concurrency limit reached" so the audit completes gracefully. Precedence: `--max-concurrency` flag > `AUDIT_MAX_CONCURRENCY` env var > 2 default.
...
**Batch deep analysis (Phase 2):** `--batch-phase2` (or the `AUDIT_PHASE2_BATCH` env var, truthy) folds the parent ACs and all pending child ACs into ONE indexed pi call (`phase2_batch`) instead of a parent call plus one call per child. If the batched call fails (provider error, timeout, or unparseable output), the runner falls back to the per-child call path so verdicts degrade exactly as in non-batch mode. Default off; the env var is overridden by an explicit `--batch-phase2` flag.
