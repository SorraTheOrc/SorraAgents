...
- **Child-audit execution contract (SA-0MU32TE120012T49):** in-main-slot mode never presents a pending child as a normal result. The report's `Ready to close:` header is immediately followed by an explicit `Child screens pending: N (main-slot) — <ids>` gate, the summary repeats it, and `Ready to close: Yes` is impossible while any child screen is pending. A resumable signal (`.worklog/audit-checkpoints/<id>.pending-screens.json`) records the outstanding child ids and is cleared automatically once none remain; perform each emitted `[AUDIT_IN_MAIN_SLOT_WORK]` screen, persist the child audit, then re-run the parent to complete exactly those children without re-running completed phases.
