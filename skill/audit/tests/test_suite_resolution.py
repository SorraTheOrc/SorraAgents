...
- AC1 (RED until F2): ``full_suite_commands`` on a TCE-like repo (npm test
  script, no pytest config, no ``tests/{unit,node,cli}`` dirs) must detect
  the ``npm test`` suite command — never a phantom pytest command.
- AC4 (RED until F2): ``full_suite_commands`` must read
  ``<project_root>/.pi/test-config.json`` when present and use its
  ``suiteCommands`` entries as the primary command list.

Both are RED against the current codebase (the phantom pytest command is
emitted unconditionally and the extension file is ignored) and go GREEN
with F2 (SA-0MSTMYE79006NA61). They are pinned ``xfail(strict=True)`` so
the suite stays green at F1's commit; when F2 lands the behavior, the tests
XPASS and the strict marker forces F2 to remove it (a passing xfail with
strict=True fails the suite).
"""
...
class TestTceLayoutDetection:
    """AC1 (RED until F2): npm-test convention detection for TCE-like repos.
    ...
    @pytest.mark.xfail(
        reason=(
            "F1 contract AC1 — npm-test detection lands in F2 "
            "(SA-0MSTMYE79006NA61); RED on current code (phantom pytest "
...
class TestExtensionFileResolution:
    """AC4 (RED until F2): the per-project extension file.
    ...
    @pytest.mark.xfail(
        reason=(
            "F1 contract AC4 — extension file resolution lands in F2 "
            "(SA-0MSTMYE79006NA61); RED on current code (extension file "
            "ignored)"
        ),
        strict=True,
    )
